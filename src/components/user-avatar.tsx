  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex cursor-pointer items-center space-x-1 rounded-full p-1.5 transition-all hover:bg-slate-100 focus:outline-none"
      >
        <div className="relative size-10">
          {user?.image ? (
            ...
            <div className="bg-primary flex h-full w-full items-center justify-center rounded-full border-2 border-white text-sm font-bold text-white shadow-sm">
              {initials}
            </div>
          )}
      ...
      {isOpen && (
        <div className="animate-in fade-in zoom-in-95 absolute right-0 z-50 mt-2 w-56 rounded-2xl border border-slate-100 bg-white py-2 shadow-xl duration-100">
          ...
          <Link
            href="#"
            className="hover:bg-accent hover:text-accent-foreground flex items-center space-x-3 px-4 py-2 text-sm text-slate-600 transition-colors"
          >
